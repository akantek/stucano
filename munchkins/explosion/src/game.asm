...
collision_demo:
  ...
  ; --- NEW: Reset States on Boot/Restart ---
  xor a                  ; Fast way to set A = 0
  ld (player_state), a   ; Reset Player to Alive
  ld (enemy_state), a    ; Reset Enemy to Alive
  ...
  xor a                  ; Fast way to make A = 0
  ld (player_state), a   ; 0 = ALIVE
  ld (enemy_state), a    ; 0 = ALIVE
...
.vblank_trace_end:
  ...
  ; --- STATE CHECK: ARE WE DEAD? ---
  ld a, (player_state)
  or a                   ; Check if 0 (Alive) or 1 (Dead)
  jr z, .play_game       ; If 0 (Alive), jump down to the normal game logic!

  ; --- DEAD LOGIC (Waiting for Restart) ---
  call scan_keypad       ; Read the keyboard
  bit 0, a               ; Check Bit 0 (Usually the Spacebar on MSX Row 8)
  jr nz, .skip_beep      ; If it is NOT pressed, jump to the end of the loop and freeze screen!
    
  jp collision_demo      ; IF PRESSED: Jump all the way back to the start and reboot the game!
  ; ==========================================

.play_game:
  ; --- Check State before moving the enemy ---
  ld a, (player_state)         ; Check if the player is alive (0) or dead (1)
  or a                         ; Wipe the Carry flag, set the Zero flag based on A
  jr nz, .skip_enemy_move      ; If Not Zero (Dead), jump right over the movement!

  ; If we survive the jump, the player is alive, so move the enemy!
  ld hl, sprite0_y
  inc (hl)
...
animate_sprite0:
  ...
  ld a, (enemy_state)    ; Check the enemy's state
  or a                   ; Is it 0 (Alive)?
  ret nz                 ; If Not Zero (Dead), return immediately! Do not animate!
...
update_player_movement:
  ...
  ld a, (player_state)   ; Check the player's state
  or a                   ; Is it 0 (Alive)?
  ret nz                 ; If Not Zero (Dead), return immediately! Ignore joypad!
