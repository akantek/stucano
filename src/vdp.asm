...
loadSpriteAttributes:
  ...
  ret z                          ; If 0 sprites, exit immediately to prevent VRAM corruption

  ; Multiply A by 4 (4 bytes per sprite)
  add a, a                       ; A = A * 2
  add a, a                       ; A = A * 4
  ld b, a                        ; Store total byte count in B for the OTIR instruction

  ; Setup source and port
  ...
  ld a, VRAM_SPR_ATTRIBS / 256   ; Load the raw high byte
